...
# --- 1. CONFIGURACIÓN (SIEMPRE LA PRIMERA LÍNEA) ---
...
# --- 3. ESTILOS (DISEÑO GOLD V50.8 - INTACTO) ---
def inject_style(mode):
    # CSS para limpiar la interfaz de Streamlit
    base_css = """
    <style>
...
# --- 4. DICCIONARIOS DE DATOS E INTELIGENCIA ---
...
# DICCIONARIO MAESTRO (ESTO ARREGLA EL LOCUS=0 Y LOS PUNTOS PERDIDOS)
KEY_TRANSLATION = {
    # Achievement
    ...
    # Locus Control (ESTO ES LO QUE ESTABA FALLANDO EN EL OTRO ARCHIVO)
...
TRAIT_TEXTS = {
    ...
    "self_efficacy": {
        "low": "ÁREA DE MEJORA: Dudas sobre la propia capacidad.",
...
# --- PARSEO INTELIGENTE (DIVISIÓN POR 5) ---
def parse_logic(logic_str):
    ...
    for part in parts:
        try:
            ...
            # AQUÍ ESTÁ EL TRUCO: DIVIDIMOS POR 5 AUTOMÁTICAMENTE
...
# --- CÁLCULO DE RESULTADOS (NORMALIZACIÓN 500) ---
def calculate_results():
    raw_traits = st.session_state.traits.copy()
    total_raw = sum(raw_traits.values())
    final_traits = {}
    
    # Si sumas más de 500 puntos, comprimimos
    if total_raw > 500:
    ...
    return round(ire, 2), round(avg, 2), round(friction, 2), triggers, trait_details, 0
...
if st.session_state.current_step == 0:
    ...
    ire, avg, friction, triggers, trait_details, _ = calculate_results()
