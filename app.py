...
# MAPA DE VARIABLES (Clasificación estricta: Traits vs Flags)
VARIABLE_MAP = {
    # --- RASGOS POSITIVOS (TRAITS) ---
    # Achievement
    ...
    "forward": "innovativeness", "imaginative": "innovativeness", # Movido aquí
...
def calculate_results():
    # 1. POTENCIAL (Suavizado Logístico)
    # Evita que se dispare a 100 rápidamente.
    raw_points = sum(st.session_state.traits.values())
    # Con 200 puntos brutos tienes un ~57/100. Con 400 puntos un ~73/100.
    avg = 100 * (1 - (1 / (1 + (raw_points / 150.0))))
    
    # 2. FRICCIÓN (Solo Flags)
    raw_friction = sum(st.session_state.flags.values())
    # 50 puntos de banderas rojas = 100% de fricción (peligroso)
    friction = min(100, (raw_friction / 50.0) * 100)
    
    # 3. IRE (Índice de Rendimiento)
    # Penaliza el potencial basándose en la fricción
    penalty = friction / 200.0 # Max penalización es 50% del potencial
    ire = avg * (1 - penalty)
...
if st.session_state.current_step == 0:
    ...
    # Normalización visual para el gráfico (0-10)
    ...
    if PDF_AVAILABLE:
        def create_pdf():
            ...
            c.drawString(50, 750, f"Diagnóstico: {get_ire_text(ire)}")
            if triggers:
                c.drawString(50, 730, f"Alertas: {', '.join(triggers)}")
            c.save()
            b.seek(0)
            return b
