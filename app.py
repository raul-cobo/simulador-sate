...
# Preguntas
    ...
    with c2:
        if st.button("A: "+str(row.get('OPCION_A_TXT',''))): parse_logic(row.get('OPCION_A_LOGIC')); st.session_state.current_step+=1; st.rerun()
        if st.button("B: "+str(row.get('OPCION_B_TXT',''))): parse_logic(row.get('OPCION_B_LOGIC')); st.session_state.current_step+=1; st.rerun()
        if row.get('OPCION_C_TXT') and len(str(row.get('OPCION_C_TXT'))) > 1: 
            if st.button("C: "+str(row.get('OPCION_C_TXT',''))): parse_logic(row.get('OPCION_C_LOGIC')); st.session_state.current_step+=1; st.rerun()
        if row.get('OPCION_D_TXT') and len(str(row.get('OPCION_D_TXT'))) > 1: 
            if st.button("D: "+str(row.get('OPCION_D_TXT',''))): parse_logic(row.get('OPCION_D_LOGIC')); st.session_state.current_step+=1; st.rerun()
