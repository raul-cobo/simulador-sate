...
def init_session():
    if 'octagon' not in st.session_state:
        # BASE 0: Confiamos en el CSV rebalanceado (ya suma 85 puntos)
    ...
    for part in logic_str.split('|'):
        try:
            ...
            if k in st.session_state.octagon:
                # Topes 0-100, suma directa (sin multiplicador)
...
def calculate_results():
    ...
    # Fricción: Dividimos por 3 (para normalizar escala de flags raw)
...
if not st.session_state.data_verified:
    ...
    def go(s):
        try:
            # LECTURA ROBUSTA: Probamos codificación 'utf-8-sig' (Excel) para evitar errores de BOM
            try:
            ...
            # Normalizamos el nombre del sector para evitar errores por mayúsculas/espacios
            st.session_state.data = [r for r in rows if r['SECTOR'].strip().upper() == s.upper()]
            
            if not st.session_state.data:
                st.error(f"⚠️ Error: No hay preguntas para '{s}' en el archivo. Revisa el CSV.")
                return
    ...
    # Si por algún motivo se sale del índice, finalizamos
    ...
    with c2:
        ...
        # Solo mostrar C y D si existen y no están vacíos
