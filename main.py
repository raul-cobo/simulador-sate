...
# ==========================================
# 2. FUNCIONES GLOBALES DE SESIÓN
# ==========================================
...
# ==========================================
# 3. PANTALLA DE LOGIN RAÍZ (CORREGIDA)
# ==========================================
@ui.page('/')
def login_page():
    ...
    with ui.column().classes('w-full h-screen items-center justify-center gap-6'):
        with ui.card().classes('bg-white p-8 rounded-2xl shadow-2xl items-center w-[30vw] min-w-[320px]'):
            ...
            # Lógica de verificación inyectando variables directamente
            async def intentar_login(u, p):
                # Protección contra variables Null
                u_val = u or ""
                p_val = p or ""
                user_str = u_val.strip()
                pwd_str = p_val.strip()
            ...
            # Botones con Lambda para capturar el valor exacto en el momento del click
...
# ==========================================
# 4. RUTAS DE LAS CONSOLAS (B2B y ADMIN)
# ==========================================
@ui.page('/admin')
def admin_page():
    ui.query('body').style(f'background-color: {BG_COLOR}; color: white; margin: 0;')
...
def panel_page():
    ...
    # --- 1. CARGA DE DATOS Y PRIVILEGIOS ---
...
# ==========================================
# 6. ENRUTAMIENTO HACIA LOS MOTORES (SAPE / SAPP)
# ==========================================
@ui.page('/sape-test')
def sape_test(sector: str = 'TECH'):
    ...
    if not app.storage.user.get('authenticated'):
        ui.navigate.to('/')
        return
    interfaz = SAPEInterface(sector_context=sector)
    interfaz.render()
